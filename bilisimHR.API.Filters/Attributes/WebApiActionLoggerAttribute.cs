    ...
    public class WebApiActionLoggerAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (actionContext.Request.Properties.ContainsKey("MS_OwinContext"))
            {
                ...
                {
                    userID = claimsIdentity.Claims.Where<Claim>(s => s.Type == ClaimTypes.PrimarySid).FirstOrDefault().Value;
                    userName = claimsIdentity.Claims.Where<Claim>(s => s.Type == ClaimTypes.Name).FirstOrDefault().Value;
                }
            }
